...
## Production приёмка EVM — 14.09.2026 19:25:10 UTC

Владелец явно разрешил account/balance read-only аудит и активацию на приватной
временной копии production БД. Аудит до и после выката прошёл: authenticated
исторические заявки и текущая позиция совпали с книгой, balanceOf покрывает
инвентарь, открытых заявок нет; shadow activation сохраняет frozen inst_json.
Новых торговых отправок аудит не выполнял. Предыдущая блокировка auto-review снята.

Установлен 0f9037cf72de88131dcbcf8129bfd9455973dcba, release
0f9037cf72de-f6e1e96f711e через deploy/deploy.sh. Base aa446f2 совпала.
Receipt: healthy; base_verified, drained, execution_released,
service_layout_proved, artifact_installed, three_process_ready.
Серверный цикл 163.759 s. Reader floor 4: возврат на прежний несовместимый reader
не допускается, старая БД поверх текущей не восстанавливается.

Первый пакет принят в заявленной области EVM execution/recovery. Это не полная
приёмка исторического PnL. Второй пакет общего координатора начат; C1–C5 ещё
не завершены. Код первого пакета опубликован и установлен, второй пока в работе.
