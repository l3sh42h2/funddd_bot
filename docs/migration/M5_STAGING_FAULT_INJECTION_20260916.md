# M5 staging fault-injection — 16.09.2026
...
(см. ниже); без вмешательства ждал бы бесконечно. `trade.db` за всё время ожидания не менялся (sha256
...
- Нюанс: `rollback_release()` в самом конце тоже вызывает `wait_components()` (для ПОДТВЕРЖДЕНИЯ,
  ...
  Код/БД-часть отката (то, что просит пункт 4) при этом доказана независимо и напрямую —
  тремя способами: journalctl + IPC + sha256, а не только по итоговому отчёту деплоя.
