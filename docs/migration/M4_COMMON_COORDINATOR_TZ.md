...
Обновление 15.09.2026: реализация и матрица — [пакет приёмки](M4_COMMON_COORDINATOR_RESULT.md).
Владелец позднее назначил итоговое ревью Claude и согласовал менее дорогие модели исполнения;
упоминания Astra ниже сохраняют исходное состояние ТЗ, не являются текущим назначением.
