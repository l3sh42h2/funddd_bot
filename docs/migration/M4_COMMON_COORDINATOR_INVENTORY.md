...
Подготовка по M4_COMMON_COORDINATOR_TZ.md. База EVM-кандидата: 0f9037c (приёмка production ещё не закрыта).
Это инвентарь, не свидетельство завершённого C1–C5. Linux build первого кандидата 0f9037c прошёл;
проверка реальной активации на серверной копии ожидает отдельного разрешения auto-review.
