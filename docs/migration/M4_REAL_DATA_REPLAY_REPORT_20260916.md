...
(адрес не приводится здесь намеренно — репозиторий сейчас публичный, см. `docs/migration/
FUNDING_REPO_PUBLIC_EXPOSURE_NOTE.md` при наличии, иначе — предупреждение в PATCHNOTES), ни `raw_tx`,
ни `owner_json` в файлах нет (whitelist отсекает сам).

**Примечание о публичности (16.09, независимая проверка):** JSON-снимки не содержат кошелька/raw_tx, но
содержат 2 реальных BSC tx_hash — этого достаточно, чтобы через блок-эксплорер связать их с тем же
кошельком и его историей. Это открытые ончейн-данные (не секрет в смысле ключей), но при пуше в
публичный репозиторий это стоит явно учитывать как решение владельца, не как техническое требование.
