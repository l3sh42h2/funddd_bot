"""Executable synthetic solana_spot adapter; native transport is supplied by the test.
...
def register(registry, key):
    registry.register(key, lambda spec, context: SolSpot(spec, context.for_leg(spec)))
