"""Executable synthetic dex_futures adapter; native transport is supplied by the test.
...
class DexFutures(NativeAdapter):
...
def register(registry, key):
    registry.register(key, lambda spec, context: DexFutures(spec, context.for_leg(spec)))
