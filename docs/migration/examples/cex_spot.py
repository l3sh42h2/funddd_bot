"""Executable synthetic cex_spot adapter; native transport is supplied by the test.
...
def register(registry, key):
    registry.register(key, lambda spec, context: CexSpot(spec, context.for_leg(spec)))
