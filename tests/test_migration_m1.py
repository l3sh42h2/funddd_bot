...
def test_drain_and_separate_deploy_authority(conns):
    ...
    # Model the unprivileged interface explicitly.  The verification profile
    # itself runs as root, while production keeps core and interface distinct.
    ui_uid = 1000
    s.ui_uid = ui_uid
    with pytest.raises(RpcError,match='unauthorized_method'):
        s.dispatch('begin_drain',{},None,ui_uid)
    s.dispatch('begin_drain',{'release_id':'fixture','expected_state_revision':0},None,0)
    with pytest.raises(RpcError,match='draining'):
        s.dispatch('submit_user_command',{'update':update(1,'продолжить')},'tg:1',ui_uid)
    assert s.dispatch('submit_user_command',{'update':update(2)},'tg:2',ui_uid)['state']=='queued'
