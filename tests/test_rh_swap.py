"""evm_swap.py на Robinhood Chain (4663): гарды отличают «адрес не подтверждён» (allowlist пуст — сеть новая) от
«OKX сменила роутер» (BSC, allowlist непуст, но адрес незнакомый) и говорят нативную монету сети своим именем
(BNB/ETH). Всё на чистых функциях/минимальных подделках — без сети, без реального /swap для 4663 (недоступен)."""
...
def test_router_unconfirmed_for_robinhood_gives_clear_message():
    ...
    msg = str(ei.value)
    assert "не подтверждён" in msg and "4663" in msg and "doctor" in msg
...
def test_spender_unconfirmed_for_robinhood_gives_clear_message():
    amount = 10 ** 6
...
def test_trim_receiver_unconfirmed_message_direct():
    ...
    assert ei.value.guard == "calldata_tail" and "не подтверждён" in str(ei.value) and "4663" in str(ei.value)
